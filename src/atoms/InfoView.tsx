...
export const InfoView = ({id, infos}: InfoViewProps) => {
    const toRow = (info: (string | number)[], index: number) => 
        <tr className={style.contextAccordinText} key={`${id}-info${index}`} style={{border: "1px solid"}}>
            <td style={{border: "1px solid", whiteSpace: "nowrap"}}>{<FormattedMessage id={info[0].toString()} />}</td>
